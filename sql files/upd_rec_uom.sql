IF ((new.rec_uom_name = 'Gram' OR new.rec_uom_name = 'ML') AND new.rec_uom_value = 1) THEN
        ...
        new.protein = (SELECT energy_kcal FROM total_recipe WHERE recipe_id = new.recipe_id) * (new.serving_measurements) / (SELECT serving_measurements FROM total_recipe WHERE recipe_id = new.recipe_id),
