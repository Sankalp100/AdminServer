(function() {
  ...
  angular.module('app4').controller('ingredientsController', ingredientsController);
  
  ingredientsController.$inject = ['$scope', '$http'];
...
function ingredientsController($scope, $http) {
  ...
  $scope.updateUser = function() {
    ...
    $http({
      ...
      method: 'PUT',
      url: 'http://localhost:80/pRESTige-master/api/ingredients/' + $scope.user.id,
      data: $scope.user
  ...
  $scope.deleteUser = function(user) {
    ...
    $http({
      ...
      method: 'DELETE',
      url: 'http://localhost:80/pRESTige-master/api/ingredients/' + user.id
